...
(()=>{
  ...
  function renderGiftCatalogV20(){
    ...
    const banner=document.getElementById('gift-limit-banner'),creatorUnlimited=creatorV20(me.nick),cooldown=!creatorUnlimited&&limitedNextV20>Date.now();banner.classList.toggle('show',limited);if(limited){const active=catalogV20.find(gift=>gift.limited&&Number(gift.available_until)>Date.now());banner.textContent=creatorUnlimited?(active?'Для creator нет паузы 21 день · коллекция закроется через '+countdownV20(active.available_until):'Коллекция закрыта'):cooldown?'Следующий лимитированный подарок будет доступен через '+countdownV20(limitedNextV20):active?'Коллекция закроется через '+countdownV20(active.available_until)+' · можно выбрать только один подарок на 21 день':'Коллекция закрыта';}
