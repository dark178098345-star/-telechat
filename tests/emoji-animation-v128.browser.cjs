...
(async()=>{
 ...
 try{
  ...
  await p.locator('.tele-emoji-v127[data-tele-emoji="❤️"]').click();assert.equal(await p.evaluate(()=>telechatEmojiMotionV128.info().active),0,'message click is not the animation trigger');await p.evaluate(()=>telechatEmojiMotionV128.animate(document.querySelector('[data-emoji-motion=heart]')));
