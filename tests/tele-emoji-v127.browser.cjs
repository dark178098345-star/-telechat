...
(async()=>{
 ...
 try{
  for(const mobile of [false,true]){
   ...
   assert.equal(await p.locator('.emoji-item-v80[data-tele-emoji]').count(),16,'own set is the default on every device');
   assert.equal(await p.locator('.emoji-item-v80 svg').count(),16);assert.equal(await p.locator('.emoji-category-v80:not([data-category=telechat]) .ui-icon-v125').count(),11);
   ...
   await p.evaluate(()=>{for(let i=0;i<30;i++)buildEmojiPicker();});assert.equal(await p.locator('.emoji-item-v80[data-tele-emoji]').count(),16,'mounting is idempotent');
   ...
   await p.locator('.msg .tele-emoji-v127').first().click();assert(await p.locator('.msg .tele-emoji-v127 svg').first().evaluate(el=>el.getAnimations().some(a=>a.constructor.name==='Animation')),'tap plays a short local animation');
  ...
  console.log('PASS custom emoji: 16 original assets, desktop/mobile picker, cursor insertion, input events, search, recents, old catalog, safe Unicode rendering, ZWJ/skin tones, semantic text, idempotence, tap/reduced-motion, shared reaction art, layouts');
