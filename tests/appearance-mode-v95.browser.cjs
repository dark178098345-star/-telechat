...
(async()=>{
 ...
 try {
  ...
  const phone=await context.newPage();await phone.setViewportSize({width:390,height:800});await phone.goto('http://telechat.test/');await phone.evaluate(()=>localStorage.removeItem('telechat_calm_mode_v95'));await phone.reload();
  const phoneMedia=()=>phone.locator('#calm-style-v95').getAttribute('media');
  assert.equal(await phoneMedia(),'all');assert(await phone.locator('#calm-mode-v95').isChecked(),'phones default to calm mode');
  await phone.locator('#calm-mode-v95').evaluate(el=>{el.checked=false;el.dispatchEvent(new Event('change',{bubbles:true}));});
  assert.equal(await phoneMedia(),'not all');await phone.reload();assert.equal(await phoneMedia(),'not all');
  await phone.evaluate(()=>localStorage.removeItem('telechat_calm_mode_v95'));await phone.reload();assert.equal(await phoneMedia(),'all');
