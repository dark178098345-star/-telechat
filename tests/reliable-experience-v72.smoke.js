...
const device = read('presence-v120.js');
const presenceModel = read('presence-model-v120.js');
...
assert.match(presenceModel, /__telechat_device_phone_v72__/);
assert.match(presenceModel, /__telechat_device_pc_v72__/);
assert.match(device, /С телефона/);
assert.match(device, /С компьютера/);

for (const asset of ['delivery-v72.css?v=72', 'presence-model-v120.js?v=120', 'presence-v120.js?v=120']) {
...
assert.match(serviceWorker, /telechat-shell-v122-safe-cleanup/);
