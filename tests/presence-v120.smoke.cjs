...
for(const asset of ['presence-model-v120.js?v=120','presence-v120.js?v=120','smooth-ui-v76.js?v=120','chat-speed-v51.js?v=121']){assert.ok(index.includes(asset));assert.ok(sw.includes(asset));}
...
assert.match(sw,/telechat-shell-v121-message-recovery/);assert.match(logout,/telechatPresenceV120\.leave/);assert.doesNotMatch(logout,/delete\(\)\.eq\('nick',user\.nick\),/,'Logout must not erase other devices or privacy markers');
