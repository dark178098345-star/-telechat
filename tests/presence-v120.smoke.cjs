...
for(const asset of ['presence-model-v120.js?v=129','presence-v120.js?v=129','smooth-ui-v76.js?v=120','chat-speed-v51.js?v=130']){assert.ok(index.includes(asset));assert.ok(sw.includes(asset));}
...
assert.match(native,/protected void onStart/);assert.match(native,/protected void onStop/);assert.match(native,/telechat-native-visibility/);assert.match(native,/isInBackground/);assert.match(native,/telechat-android\/1\.2\.5/);
