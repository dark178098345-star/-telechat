...
(async () => {
  ...
  telechatChatSpeedV51.clearConversation('creator_tele');
  assert.equal(messages.children.length, 0, 'clearing a chat must discard its hot history cache and DOM');
  await appendMessage({ id: 3, chat_key: 'creator_tele', from_nick: 'creator', text: 'fresh', ts: 300, read_by: [] });
  assert.deepEqual(messages.children.filter(element => element.classList.contains('msg')).map(element => element.dataset.id), ['3'], 'cleared messages must not return after a new message');
